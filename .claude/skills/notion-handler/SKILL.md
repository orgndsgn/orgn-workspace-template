...
Notion의 File Upload API를 사용해 로컬 파일을 Notion 스토리지에 직접 업로드합니다 (external URL 아님).

#### 파일 업로드 (upload_id 반환)
```bash
python3 .claude/skills/notion-handler/scripts/notion_api.py upload-file --path "/local/path/file.png"
# 출력: {"id": "<upload_id>", "name": "file.png", "content_type": "image/png"}
```

#### files 프로퍼티에 업로드한 파일 첨부
페이지 생성/수정 시 `files` 프로퍼티에 raw Notion format으로 전달:

```bash
# 1) 먼저 업로드
UPLOAD_ID=$(python3 .claude/skills/notion-handler/scripts/notion_api.py upload-file --path "/tmp/thumb.png" | jq -r .id)

# 2) 페이지 생성 시 files 프로퍼티에 참조
python3 .claude/skills/notion-handler/scripts/notion_api.py create-page \
  --parent "DB_ID" \
  --properties "{
    \"제목\": \"영상 제목\",
    \"섬네일\": {\"files\": [{\"type\": \"file_upload\", \"file_upload\": {\"id\": \"$UPLOAD_ID\"}, \"name\": \"thumb.png\"}]}
  }"
```

#### 외부 URL로 files 프로퍼티 채우기
업로드 없이 외부 URL만 참조하려면:
```json
"섬네일": {"files": [{"type": "external", "external": {"url": "https://..."}, "name": "thumb.png"}]}
```
...
| `clear-page` | 페이지 초기화 |
| `upload-file` | 로컬 파일을 Notion 스토리지에 업로드 (upload_id 반환) |
| `search` | 검색 |
...
- **v1.2.0 (2026-05-16)**: 파일 업로드 지원 (upload-file 명령어, files 프로퍼티 raw 패스스루). create-page/update-page가 DB 스키마를 자동 조회해 타입 매핑을 스키마 기반으로 처리 (필드명 하드코딩 의존 제거).
