...
> **Updated**: 2026-05-16 (v2.1.143) - CLAUDE.md 직접 변경 없음. 관련: compaction 시 사용자 지침 보존(v2.1.139), Rewind "Summarize up to here"(v2.1.141), Fast mode → Opus 4.7 (v2.1.142)
...
- **(v2.1.139)** Compaction prompt가 모델에게 **민감한 사용자 지침을 보존**하도록 명시. CLAUDE.md/Auto Memory 지침이 auto-compaction 이후에도 더 잘 살아남음.
- **(v2.1.141)** Rewind 메뉴에 **"Summarize up to here"** 추가 — 과거 턴을 압축하면서 최근 턴은 그대로 유지. CLAUDE.md 컨텍스트 보존 + 토큰 절약 양립.
- **(v2.1.142)** Reactive compaction 첫 시도가 원본 요청의 overflow 크기에서 시작해 near-full-context retry 낭비 제거.
- **(v2.1.142)** Fast mode 기본 모델: Opus 4.6 → **Opus 4.7**. 4.6 고정: `CLAUDE_CODE_OPUS_4_6_FAST_MODE_OVERRIDE=1`
- **(v2.1.119)** 스킬이 auto-compaction 직전 호출되면 다음 사용자 메시지에 재실행되던 버그 수정 — CLAUDE.md에서 자동 트리거되는 스킬에 영향.
...
- **2026-05-16**: v2.1.120~2.1.143 — CLAUDE.md 시스템 직접 변경 없음. 주변 컨텍스트 메커니즘 영향:
  - Compaction prompt가 사용자 지침 보존 (v2.1.139) — CLAUDE.md 지침이 압축 이후에도 잘 유지됨
  - Rewind 메뉴 "Summarize up to here" (v2.1.141) — 부분 압축 옵션
  - Reactive compaction 효율 개선 (v2.1.142)
  - Fast mode 기본 모델 Opus 4.6 → 4.7 (v2.1.142)
