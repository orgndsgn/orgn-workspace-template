...
> **Updated**: 2026-05-16 (v2.1.143) - `${CLAUDE_EFFORT}` 스킬 substitution, `skillOverrides` 설정, `Skill(name *)` 와일드카드 prefix 매치, 서브에이전트 skill 발견, 플러그인 root-level SKILL.md 지원
...
**Plugin SKILL.md 자동 surfacing (v2.1.142)**: `skills/` 서브디렉토리 없이 플러그인 루트에 `SKILL.md`만 있으면 자동으로 단일 스킬로 인식. `skills: ["./"]` 선언으로 플러그인 루트 자체를 스킬 디렉토리로 지정 가능 (v2.1.142에서 "path escapes plugin directory" 거짓 오류 수정).
...
- **(v2.1.121)** `/skills` 메뉴에 **type-to-filter 검색 박스** 추가 — 긴 목록에서 스크롤 없이 검색 가능
- **(v2.1.139)** `/context` per-skill 토큰 추정치가 모델 토크나이저를 반영해 정확한 반올림 값으로 표시
...
| `${CLAUDE_EFFORT}` | **(v2.1.120)** 현재 effort 수준 (`low`/`medium`/`high`). 스킬 콘텐츠가 effort에 따라 동작을 분기할 때 사용. |
...
- **(v2.1.133)** 서브에이전트도 `Skill` 도구를 통해 project/user/plugin 스킬을 직접 발견·호출 가능 (이전에는 누락되던 버그가 수정됨). `skills:` 프리로드 없이도 사용 가능.
...
- `Skill(name *)`: 접두사 매치 (인자 포함). **(v2.1.139 수정)** 와일드카드 형식이 `Bash(ls *)`와 동일한 prefix-match로 정상 동작하도록 fix.

`user-invocable`은 메뉴 표시만 제어. 프로그래밍 방식 호출 차단은 `disable-model-invocation: true` 사용.

### skillOverrides 설정 (v2.1.129)

`~/.claude/settings.json`의 `skillOverrides`로 개별 스킬의 노출 범위 제어:

```jsonc
{
  "skillOverrides": {
    "verbose-skill": "name-only",         // 설명은 collapsed, 이름만 노출
    "internal-helper": "user-invocable-only", // 모델 자동 호출 차단, /-메뉴는 유지
    "deprecated-skill": "off"             // 모델과 /-메뉴 모두에서 숨김
  }
}
```
...
|----|---------------|---------|----------|
| `off` | X | X | - |
| `user-invocable-only` | X | O | description |
| `name-only` | O | O | 이름만 (description 미로드) |

description이 너무 긴 스킬을 일시 숨김 처리할 때 유용.

### Permission bypass 동작 (v2.1.121)

`--dangerously-skip-permissions`는 `.claude/skills/`, `.claude/agents/`, `.claude/commands/` 디렉토리 쓰기 작업을 더 이상 prompt하지 않음. (v2.1.128에서 `.claude/`, `.git/`, `.vscode/`, shell config 등으로 확대)

### OpenTelemetry (v2.1.126)

`claude_code.skill_activated` 이벤트가 모든 호출 경로에서 발생하며 `invocation_trigger` 속성 추가:
- `"user-slash"` — 사용자가 `/skill-name` 입력
- `"claude-proactive"` — Claude가 자동 판단 호출
- `"nested-skill"` — 다른 스킬에서 호출
...
- **2026-05-16**: v2.1.120~2.1.143 변경사항 반영
  - `${CLAUDE_EFFORT}` 스킬 substitution 추가 (v2.1.120) — effort 수준에 따라 스킬 콘텐츠 분기
  - `/skills` type-to-filter 검색 박스 추가 (v2.1.121)
  - `--dangerously-skip-permissions`가 `.claude/skills/`, `.claude/agents/`, `.claude/commands/` 쓰기 prompt 생략 (v2.1.121)
  - `claude_code.skill_activated` OTel 이벤트가 user-slash 호출에도 발생, `invocation_trigger` 속성 추가 (v2.1.126)
  - `skillOverrides` 설정 정상 작동 (v2.1.129) — `off`/`user-invocable-only`/`name-only`
  - 서브에이전트가 project/user/plugin 스킬을 Skill 도구로 발견·호출 가능 (v2.1.133 fix)
  - `/context` per-skill 토큰 추정치가 모델 토크나이저 반영 (v2.1.139)
  - `Skill(name *)` 와일드카드가 정상 prefix-match로 동작 (v2.1.139 fix)
  - Skill 인자에 regex metacharacter 포함 시 argument substitution 실패하던 버그 수정 (v2.1.139)
  - 플러그인 루트의 `SKILL.md`가 단일 스킬로 자동 surfacing (v2.1.142)
  - `skills: ["./"]` 사용 시 false "path escapes plugin directory" 오류 수정 (v2.1.142)
